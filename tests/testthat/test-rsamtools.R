test_that("rsamtools works", {
    ...
    dat <- echodata::BST1
    ## Detect column names (handle both echolocatoR and MungeSumstats formats)
    chrom_col <- intersect(c("CHR","CHROM","seqnames"), colnames(dat))[1]
    pos_col <- intersect(c("POS","BP","pos","start"), colnames(dat))[1]
    gr <- GenomicRanges::GRanges(
        seqnames = as.integer(dat[[chrom_col]]),
        ranges = IRanges::IRanges(
            start = as.integer(dat[[pos_col]]),
            end = as.integer(dat[[pos_col]])
        )
    )
    ...
    ### Currently working
    ## Detect column positions for indexTabix
    fullSS_header <- colnames(data.table::fread(target_path, nrows = 0))
    fullSS_chrom_col <- intersect(c("CHR","CHROM","seqnames"), fullSS_header)[1]
    fullSS_pos_col <- intersect(c("BP","POS","pos","bp"), fullSS_header)[1]
    seq_idx <- which(fullSS_header == fullSS_chrom_col)
    start_idx <- which(fullSS_header == fullSS_pos_col)
    ...
    tbi <- echotabix::index(bgz_file = bgz3,
                            chrom_col = fullSS_chrom_col,
                            start_col = fullSS_pos_col,
                            method = "rsamtools")
